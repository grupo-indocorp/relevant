...
class RUC20 extends Model
{
    use HasFactory;

    protected $connection = 'mysql_flask';
    protected $table = 'ruc20_febrero28';
    ...
    /**
     * Scope para filtrar por número mínimo de trabajadores
     */
    public function scopeByMinTrabajadores($query, $minTrabajadores)
    {
        return $query->where('trabajadores', '>=', $minTrabajadores);
    }

    /**
     * Scope para filtrar por número mínimo de anexos
     */
    public function scopeByMinAnexos($query, $minAnexos)
    {
        return $query->where('anexos', '>=', $minAnexos);
    }
            ...
            if (!empty($value)) {
                switch ($key) {
                    ...
                    case 'distrito':
                        ...
                        break;
                    case 'min_trabajadores':
                        $query->byMinTrabajadores($value);
                        break;
                    case 'min_anexos':
                        $query->byMinAnexos($value);
                        break;
                }
            }
        }
    ...
    /**
     * Scope para filtrar por fuente de datos (detecta columna `fuente_datos` o `source`)
     */
